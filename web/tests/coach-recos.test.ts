...
describe('J-08 warmIngredientFreqFromSql — 런타임 warm(재배포 0·I-01-9 무손상)', () => {
  const mockDb = (rows: { ingredients: string[] }[]) => ({
    from: () => ({ select: () => Promise.resolve({ data: rows, error: null }) }),
  });
  afterEach(() => resetIngredientFreqWarm());

  it('J-08-1 warm 성공 → ingredientGioFreq가 SQL 빈도 반영(라이브와 뒤집힘)', async () => {
    const rows: { ingredients: string[] }[] = [];
  ...
  it('J-08-2 SEASONING 제외(마늘·소금 미카운트)', async () => {
    const rows: { ingredients: string[] }[] = [];
    for (let i = 0; i < 25; i++) rows.push({ ingredients: ['마늘', '소금', `채소${i}`] });
    await warmIngredientFreqFromSql(mockDb(rows));
    expect(ingredientGioFreq('마늘').freq).toBe(0);   // warm에 없음 → 폴백
  });
  it('J-08-3 빈약(<20종)·에러는 스냅샷 유지(safe degrade)', async () => {
    expect((await warmIngredientFreqFromSql(mockDb([{ ingredients: ['당근'] }]))).ok).toBe(false);
  ...
  it('J-08-4 warm 후에도 GIO_FREQ(정적 권위표·I-01-9 비교 대상) 무변경', async () => {
    const rows: { ingredients: string[] }[] = [];
    for (let i = 0; i < 25; i++) rows.push({ ingredients: [`x${i}`, '당근'] });
    await warmIngredientFreqFromSql(mockDb(rows));
