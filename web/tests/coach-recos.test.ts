...
describe('J-03 youaRankOf 별칭 — 표준명 불일치 봉합(% 날조 0)', () => {
  ...
  it('J-03-3 연어는 실측 식단 baseline에 희소(12.6%·게이트 미만) → "자주 나온다" 주장 안 함(정직)', () => {
    const r = youaRankOf('연어');   // 실측 OCR 병합 후: 18/143 기관-월 = 12.6% (과거 의도적 null → 실데이터로 대체)
    expect(r).not.toBeNull();
    expect(r!.pct).toBeLessThan(50);   // 게이트(50%) 미만 → evid 절 생략(과장 0·정직 유지)
  });
  it('J-04-1 [속성] 모든 대표가 youa 해소(실측 baseline 병합 후 null 0)', () => {
...
describe('J-05 빈도 가중 — 라이브 우선 + youa 동률 가산(골든 보존)', () => {
  it('J-05-1 ingredientGioFreq — 라이브(실측 식단) 우선(당근 1938/5)·미수록은 폴백', () => {
    expect(ingredientGioFreq('당근')).toEqual({ freq: 1938, pct: 5 });   // 라이브(실측 식단 baseline)이 GIO 스냅샷 덮어씀
    expect(ingredientGioFreq('요거트').freq).toBe(0);                    // 라이브 침묵(요구르트가 표준명) → GIO 폴백
...
describe('J-06 급식 근거 — 안심 톤만, 서열·% 금지(이사님 2026-06-19)', () => {
  ...
  it('J-06-2 희소 재료(연어 12.6%·게이트 미만)는 "자주 오른다" 주장 안 함(정직)', () => {
    const t = buildRecoFacts({ likedIngredients: [], target: '생선류', targetIngredient: '연어', freqMap: FM }).text;
    expect(t).toContain('연어');
  ...
  it('J-06-4 실측 baseline로 단호박(실측 65.7%) 안심 절 복권(표준식단 1.4% 게이트 탈락 → 실측 통과·증상② 회귀 락)', () => {
...
describe('J-08 warmIngredientFreqFromSql — (기관,월) dedup warm(I-01-9 무손상)', () => {
  type Row = { institution_menu_id: string; ingredients: string[] };
  const mockDb = (rows: Row[]) => ({
    from: () => ({ select: () => Promise.resolve({ data: rows, error: null }) }),
  });
  afterEach(() => resetIngredientFreqWarm());

  it('J-08-1 (기관,월) dedup — 같은 institution_menu_id 중복 1회, 다른 기관-월은 누적', async () => {
    const rows: Row[] = [];
    rows.push({ institution_menu_id: 'A', ingredients: ['브로콜리', '당근'] });
    rows.push({ institution_menu_id: 'A', ingredients: ['브로콜리', '감자'] });   // A에 브로콜리 2번 → 1회만
    for (const id of ['B', 'C', 'D', 'E']) rows.push({ institution_menu_id: id, ingredients: ['브로콜리'] });
    for (let i = 0; i < 20; i++) rows.push({ institution_menu_id: `G${i}`, ingredients: [`재료${i}`] });   // 분모 20종+
    const r = await warmIngredientFreqFromSql(mockDb(rows));
    expect(r.ok).toBe(true);
    expect(isFreqWarmed()).toBe(true);
    expect(ingredientGioFreq('브로콜리').freq).toBe(5);   // A(중복무시)+B+C+D+E = 5 기관-월
    expect(ingredientGioFreq('브로콜리').pct).toBeLessThan(ingredientGioFreq('재료0').pct);   // 더 흔함=상위%↓
  });
  it('J-08-2 SEASONING 제외(마늘·소금 미카운트)', async () => {
    const rows: Row[] = []; for (let i = 0; i < 25; i++) rows.push({ institution_menu_id: `M${i}`, ingredients: ['마늘', '소금', `채소${i}`] });
    await warmIngredientFreqFromSql(mockDb(rows));
    expect(ingredientGioFreq('마늘').freq).toBe(0);   // warm에 없음 → 폴백
  });
  it('J-08-3 빈약(기관-월<3 또는 식재료<20)·에러는 스냅샷 유지(safe degrade)', async () => {
    const sparse = await warmIngredientFreqFromSql(mockDb([{ institution_menu_id: 'A', ingredients: ['당근'] }, { institution_menu_id: 'B', ingredients: ['두부'] }]));   // 기관-월 2<3
    ...
    expect(ingredientGioFreq('당근')).toEqual({ freq: 1938, pct: 5 });   // 라이브 스냅샷 그대로(실측 식단·warm 실패해도 불변)
  });
  it('J-08-4 institution_menu_id 없는 고아 행은 무시(과대계상 방지)', async () => {
    const rows: Row[] = []; for (let i = 0; i < 22; i++) rows.push({ institution_menu_id: `M${i}`, ingredients: [`재료${i}`, '시금치'] });
    rows.push({ institution_menu_id: '', ingredients: ['오크라', '오크라', '오크라'] });   // 키 없음 → 무시(오크라=GIO/라이브 미수록)
    await warmIngredientFreqFromSql(mockDb(rows));
    expect(ingredientGioFreq('시금치').freq).toBe(22);   // warm이 GIO(13)를 덮음
    expect(ingredientGioFreq('오크라').freq).toBe(0);    // 고아 무시 → warm·라이브·GIO 어디에도 없음
  });
  it('J-08-5 warm 후에도 GIO_FREQ(정적 권위표·I-01-9 비교 대상) 무변경', async () => {
    const rows: Row[] = []; for (let i = 0; i < 25; i++) rows.push({ institution_menu_id: `M${i}`, ingredients: [`x${i}`, '당근'] });
    await warmIngredientFreqFromSql(mockDb(rows));
