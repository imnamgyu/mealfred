...
const NOISE = /^(소스|양념장?|쌈장|저염쌈장|초장|드레싱|마요네즈|머스타드|간장|케첩|케찹|고춧가루|참깨|깨소금|소금|설탕|물엿)$|차$|주스|에이드|식혜|수정과|미숫|음료|후기|제공|행사|체험|숲|칼슘|에너지|^단백질$|^비타민[^가-힣]?$|국산|원산지|이유식|유아식|^중기$|쿠키|약과|젤리|사탕|초콜|아이스크림|빙수|키즈|데이$|제철|특식|생일|기념|^\(|^\*|단무지|새참|있습니다|소떡|^코코아$|식이섬유|비타플러스|^반찬|가능한$|맛을$|^[0-9·\-\s]+$|양념|육수$|소스$|타르타르|^기본|피클$|장아찌$|^후식|아이스슈|마카롱|푸딩$|파이$|라떼$|^야쿠르트|^짜요|바라떼/;
