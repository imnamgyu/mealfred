...
const G: Record<string, Set<string>> = { 곡물: SET('멥쌀 찹쌀 보리 밀 빵 국수 파스타 당면 시리얼 감자 고구마 옥수수 연근 우엉 도라지 더덕 마 토란 현미 흑미 기장 수수 잡곡 귀리 도토리묵 청포묵 메밀묵 묵'), 콩류: SET('두부 검은콩 콩 콩나물 대두 완두 강낭콩 숙주나물 녹두 병아리콩 렌틸콩'), '고기·계란': SET('소고기 쇠고기 돼지고기 닭고기 오리고기 달걀 계란 메추리알 햄 베이컨 소시지 닭간'), '생선·해산물': SET('고등어 갈치 명태 대구 삼치 가자미 가다랑어 멸치 새우 오징어 게 낙지 바지락 홍합 조개 어묵 게맛살 미역 다시마 김 파래 매생이 임연수어 도미 전복 주꾸미 관자 광어 우럭 꽁치 조기 꼬막 톳 방어 미꾸라지'), 유제품: SET('우유 치즈 요구르트 요거트 크림'), 과일: SET('사과 딸기 포도 키위 참외 바나나 귤 배 감 파인애플 오렌지 멜론 복숭아 블루베리 수박 레몬 자두 망고 살구 무화과 체리 석류'), 버섯: SET('양송이버섯 느타리버섯 표고버섯 팽이버섯 새송이버섯 목이버섯 버섯') };
...
async function main() {
  ...
  const menuCat = (n: string, g: string) => {
    ...
    if (/미트볼|함박|동그랑땡|돈가스|돈까스|까스|폭찹|폭립|불고기|제육|탕수육|너겟|스테이크|데리야끼|떡갈비|커틀릿|찜닭/.test(n)) return '메인반찬';   // 명백 메인요리 이름 우선(주재료 매핑 누락·g 오판 보정)
    if (/무침|나물|샐러드/.test(n)) return '밑반찬';   // 무침·나물은 과일이 섞여도 반찬(디저트 오분류 방지)
    if (g === '과일' || g === '유제품' || /찐고구마|찐감자|군고구마|찐단호박|구운감자|찐옥수수|두유|미숫|선식|떡(?!갈비|볶이|국|꼬치|만두)|빵|케이크|샌드|시루|증편|카스테라|머핀|쿠키|푸딩|젤리|화채|꿀떡|약과|식혜|치즈스틱|핫케이크|와플|또띠아|스콘|마들렌|타르트|파이|도넛|아이스|에이드|쉐이크/.test(n)) return '디저트';
    ...
    if ((g === '고기·계란' || g === '생선·해산물' || g === '콩류') && /구이|조림|볶음|찜|장조림|강정|수육|불고기|제육|갈비|너겟|까스|돈가스|동그랑땡|전$|적$|스테이크|함박|미트볼|탕수|데리야끼|마요|튀김|볼$|커틀릿|꼬치|말이|순살|찜닭/.test(n)) return '메인반찬';
    return '밑반찬';
  };
  const mall: any[] = [];
