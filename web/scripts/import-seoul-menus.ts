...
async function main() {
  ...
  for (const k of Object.keys(byFolder)) {
    ...
    for (const month of Object.keys(byMonth)) {
      ...
      const totalScore = sevenAxisScore(axes);   // ⭐ 공식 총점 = 7축 가중 평균(단일 산식 sevenAxisScore·이사님 2026-06-24)
      ...
      const score: Record<string, unknown> = { institution_id: inst.id, month, type, sido: inst.sido, sigungu: inst.sigungu, score: totalScore, diversity_base: sc.diversityBase, gate_cap: sc.gateCap, processed: sc.processed, repeat_pen: sc.repeat, red_groups: sc.redGroups, day_count: sc.dayCount, item_count: sc.itemCount, computed_at: new Date().toISOString() };
