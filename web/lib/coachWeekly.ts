...
JSON만: {"primary_lever":"food"|"environment"|"autonomy"|"texture","mission_target":"콩류","secondary_axis":"식사환경"|null,"expose":2,"push":1,"mission":"이번 주는 ...","impression":"지난주 ... 이번 주는 ...","behavior_goal":"하루 한 끼는 ...","impl_intention":"저녁 6시, TV 끄고"}`;
...
export type WeeklySynthesis = { mission: string | null; mission_target: string | null; target_pool: string[]; secondary_axis: string | null; budget: WeeklyBudget; impression: string | null; source: string; behaviorGoal: string | null; teachingArc: TeachingArc | null; checkMethod: Record<string, unknown> | null };
...
export async function runWeeklyPlanning(i: WeeklyInput): Promise<WeeklySynthesis> {
  ...
  try {
    ...
    const lever: WeeklyLever = (['food', 'environment', 'autonomy', 'texture'] as const).includes(out.primary_lever as WeeklyLever) ? out.primary_lever as WeeklyLever : 'food';
    ...
    return {
      ...
      source: 'weekly_llm',
      behaviorGoal, teachingArc: { stages: ['why', 'reinforce'], implIntention }, checkMethod: defaultCheck(lever),
    };
  } catch {
...
function coldSynth(i: WeeklyInput, source: string): WeeklySynthesis {
  ...
  return {
    ...
    behaviorGoal: defaultBehaviorGoal('food', target), teachingArc: { stages: ['why', 'reinforce'], implIntention: null }, checkMethod: defaultCheck('food'),
  };
}
