...
export const SYSTEM_BRAIN =
  ...
  '가장 도움이 될지"만 고른다(작문·영양평가·음식추천은 다른 엔진이 한다). 원칙: ① 계산된 수치(결핍·환경·거부)는 ' +
  '사실 — 없는 문제를 지어내지 마라 ② 최근 3주 주간계획의 큰 방향을 존중하되(같은 진짜 문제면 같은 시나리오 유지 ' +
  'OK) 최근 편지가 이미 쓴 각도·무브와는 겹치지 마라 ③ 진짜 문제가 환경이면 음식 시나리오를 억지로 고르지 마라(반대도) ' +
  '④ 후보 시나리오 목록 안에서만 고른다.';
...
오늘 편지의 '행동 선택 + 추천 검수'만 JSON으로(작문 금지):
{
  ...
  "useFood": "오늘 음식 추천을 편지에 쓸지 boolean — 진짜 문제가 환경이고 음식 결핍이 약하면 false(억지 음식 금지)",
  "approvedRecs": ["위 후보 중 오늘 맞는 것만 그대로 골라 배열(부분집합·후보 밖 금지). useFood=false면 빈 배열"],
  "why": "시나리오 선택 + 추천 취사 근거 1~2문장(수치·3주 흐름·최근 편지 비겹침 인용)"
}`;
}
