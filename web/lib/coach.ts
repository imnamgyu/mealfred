...
export function buildLetterUserB(b: LetterInput): string {
  ...
  if (b.onboardingMode) {
    ...
    const fixBlock = b.fixNotes?.length
      ...
      : '';
    return `[온보딩 — 기록이 ${days}일뿐이라 분석하지 않는다. ⚠️ 결핍·패턴·영양 분석을 절대 하지 마라(데이터가 없어 환각이 된다). 부족 영양소·식품군·시계열 사실을 지어내지 마라.]
아이: ${name} (${age})
오늘 편지의 역할은 두 가지뿐이다:
...
- letter: 3~4문장. 담을 요소: ⓐ 따뜻한 인정/공감 · ⓑ '식단 거울'을 너의 표현으로 풀어 쓴 1~2문장(byte 복사 금지·사실 보존) · ⓒ '오늘의 재료'·'코칭 방향'에 맞춘 오늘의 행동 1개(P7). 도입·순서·톤은 매일 자유롭게 — 고정하지 마라.
