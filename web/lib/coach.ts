...
· 3~4문장, 짧고 따뜻하게.]\n`
  ...
  const mirrorBlock = (() => {
    ...
    let phrase: string;
    if (allMiss.length) phrase = `전체적으로 ${allMiss.join('·')} 식품군이 좀 부족해서, 집에서 조금씩 채워주면 좋아요`;
    else if (b.attendsDaycare && homeMiss.length) phrase = `어린이집 덕에 전체 영양은 잘 채워지고 있고, 집 끼니엔 ${homeMiss.join('·')}가 좀 드무니 가끔 집에서도 만나면 다양성에 좋아요`;
    else phrase = `식품 다양성은 전반적으로 잘 채워지고 있어요`;
    return `\n[⚠️ 영양거울 — 이 평가를 편지에 '한 구절'로 반드시 자연스럽게 녹여라(P10 톤·강요/점수/등급 금지). 충족 식품군(집+기관 전체): ${cov.join('·') || '기록 적음'}]\n${phrase}\n`;
  })();
  return `${history}[이번 주 상황 — 아래 사실만 사용. 영양/과거/숫자를 추가로 지어내지 말 것]
${ctx}
${factsBlock}${mirrorBlock}${scenarioBlock}${planTargetBlock}${openBlock}${actionBlock}${arcBlock}${chronicBlock}${snackBlock}${moveBlock}${regenBlock}${fixBlock}${reengageBlock}${safetyBlock}
작성 지침:
