...
export type LetterInput = {
  ...
  weeklyArc?: { stage: 'why' | 'reinforce'; behaviorGoal: string; implIntention?: string | null } | null;   // ⭐ 주간 코칭 커리큘럼(부모 행동변화·가르치는 단계) — planFromWeekly가 진척으로 결정. 부모에 '미션' 단어 노출 금지(톤만).
...
결론: 매일 새로운 관점·새 표현. 동어반복은 잔소리이며 금지.\n`
  ...
  const actionBlock = sid && SCEN_ACT[sid]
    ...
    : '';
  // ⭐ 주간 코칭 커리큘럼(§14) — 이번 주 부모 행동변화를 며칠에 걸쳐 가르치는 톤 레이어(새 행동 만들지 않음·톤만). §13 '미션' 단어 노출 금지.
  const arcBlock = b.weeklyArc
    ? (b.weeklyArc.stage === 'why'
      ? `\n[이번 주 코칭 방향 — 톤만, 새 행동 만들지 마라] 이 부모가 이번 주 시도하면 좋은 작은 변화: '${b.weeklyArc.behaviorGoal}'${b.weeklyArc.implIntention ? ` (${b.weeklyArc.implIntention} 같은 구체적 순간에)` : ''}. 오늘은 그게 '왜' 아이에게 도움 되는지 부드럽게 와닿게 하고 부모 탓이 아님을 안심시켜라. 위 행동은 권유 1번으로만 가볍게. ❌ '미션·과제·목표·이번 주 챌린지' 단어 금지. '왜 안 하셨어요' 류 금지.\n`
      : `\n[이번 주 코칭 방향 — 톤만, 새 행동 새로 얹지 마라] 이 부모가 이번 주 '${b.weeklyArc.behaviorGoal}'를 시도하기 시작한 것으로 보인다. 오늘은 같은 작은 걸음을 유지하되 그 작은 변화를 콕 집어 따뜻하게 칭찬해 굳혀라(비압박 강화). ⚠️ 데이터에서 실제 관측되지 않은 칭찬은 지어내지 마라. ❌ '미션·과제' 단어 금지.\n`)
    : '';
