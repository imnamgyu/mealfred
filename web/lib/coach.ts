...
결론: 매일 새로운 관점·새 표현. 동어반복은 잔소리이며 금지.\n`
  ...
  const slotMandateCommon = `\n❌ 두부·콩류·유부 등 다른 음식을 오늘의 음식 제안으로 대체하지 마라(어제까지 반복된 도돌이표다). '검증된 추천' 목록의 사촌·궁합은 이 음식을 보조할 때만 쓰고, 그것들로 본문 음식 제안을 대체하지 마라.\n`;
...
· 3~4문장, 짧고 따뜻하게.]\n`
  ...
  const softSlotBlock = (!(b.slotFood && b.slotDish) && b.softSlotDish)
    ? `\n[오늘 곁들일 음식 — 가볍게 한 번만] 본문 주제는 식사 환경(자리·분위기 등)이다. 그래도 본문 어딘가에 '${b.softSlotDish}' 같은 음식을 딱 한 번 가볍게 곁들여 권하라(예: "자리를 잡고 나면, 잘 먹는 음식 옆에 ${b.softSlotDish}도 한 술 같이 올려보세요"). ⚠️ 음식이 편지의 주제가 되면 안 된다(환경 코칭이 주) · 두부·콩류로 대체 금지.\n`
    : '';
  return `${history}[이번 주 상황 — 아래 사실만 사용. 영양/과거/숫자를 추가로 지어내지 말 것]
${ctx}
${factsBlock}${mirrorBlock}${growthBlock}${scenarioBlock}${planTargetBlock}${slotMandateBlock}${softSlotBlock}${openBlock}${actionBlock}${arcBlock}${chronicBlock}${snackBlock}${moveBlock}${regenBlock}${fixBlock}${reengageBlock}${safetyBlock}
작성 지침:
...
export async function composeLetter(p: {
  ...
  try {
    ...
    if (!(p.base.slotFood && p.base.slotDish) && p.base.softSlotDish && timeLeft()) {
      ...
      if (!softWoven(gen.letter)) {
        const g = await generateLetter({ ...letterInput, fixNotes: [`본문 주제는 식사 환경(자리·분위기)이되, '${p.base.softSlotDish}' 같은 음식을 딱 한 번 가볍게 곁들여 언급하라(잘 먹는 음식 옆에 한 술 올리기 식). 음식이 주제가 되면 안 되고, 두부·콩류로 대체하지 마라.`] }, model);
        if (!detBad(g.letter) && simBadness(g.letter) < 1 && softWoven(g.letter)) { gen = g; coachRegen = true; }
      }
    }
