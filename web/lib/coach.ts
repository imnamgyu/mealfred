...
검증된 추천(테이블 근거 — '[오늘 타깃] 식재료→인기 음식' + 잘 먹는 식재료→사촌·궁합. 음식·사촌·궁합은 이 목록 안에서만 골라라.${b.slotFood ? ` ⚠️ 오늘 권할 음식은 아래 '[오늘 권할 음식]'의 '${b.slotDish}' 하나다 — 이 음식은 '잘 먹는 음식의 사촌(도전)'이라 부족 식품군(콩류 등)과 달라도 정상이다(부족군은 영양거울에서 별개 정보로만 다루고, 음식 제안을 부족군으로 끌어오지 마라).` : ` ⚠️ 추천 식재료는 '오늘 부족 식품군(타깃)'에 속한다 — "○○ 채소가 부족하다"면서 달걀·고기처럼 다른 식품군을 권하지 마라: 부족 프레이밍과 추천 식재료의 식품군을 반드시 일치시켜라.`}):
${b.bridgeFacts || '파악 중'}
...
결론: 매일 새로운 관점·새 표현. 동어반복은 잔소리이며 금지.\n`
  ...
  const slotMandateBlock = (b.slotFood && b.slotDish)
    ? `\n[⚠️⚠️ 오늘 권할 음식 — 코드가 확정함, 반드시 본문에 직조] 오늘 부모에게 권할 구체 음식은 '${b.slotDish}'(재료: ${b.slotFood}) 하나다. 본문의 음식 제안(곁들임·섞기·또래 인기 메뉴)은 반드시 이 음식으로 하라 — 잘게 섞어 첫 노출이든 "또래들은 ${b.slotDish}로도 즐겨요"든, '${b.slotDish}' 또는 '${b.slotFood}'를 본문에 한 번 등장시켜라.\n❌ 두부·콩류·유부 등 다른 음식을 오늘의 음식 제안으로 대체하지 마라(어제까지 반복된 도돌이표다). '검증된 추천' 목록의 사촌·궁합은 이 음식을 보조할 때만 쓰고, 그것들로 본문 음식 제안을 대체하지 마라.\n`
    : '';
...
· 3~4문장, 짧고 따뜻하게.]\n`
  ...
  const mirrorBlock = (() => {
    ...
    if (b.mirrorPlanned) {
      if (!b.mirrorLine) return '';   // 쿨다운 — 오늘은 결핍 거울 생략
...
${ctx}
${factsBlock}${mirrorBlock}${growthBlock}${scenarioBlock}${planTargetBlock}${slotMandateBlock}${openBlock}${actionBlock}${arcBlock}${chronicBlock}${snackBlock}${moveBlock}${regenBlock}${fixBlock}${reengageBlock}${safetyBlock}
작성 지침:
