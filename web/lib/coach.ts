...
` : ''}부모 메모: ${fenceNotes(b.notes || [])}`;
  ...
  const history = past.length
    ? `\n[지난 코칭 편지 — ⚠️중복 절대 금지. 날짜·경과일 계산 금지]\n${past.map((p, i) => `[${i === 0 ? '직전(어제)' : i + 1 + '번째 전'} 편지] ${p.letter}`).join('\n')}\n위 최근 2일 편지와 **같은 행동 제안·같은 식재료·같은 칭찬 문구를 반복하지 마라**. 매일 새로운 관점·새 내용이어야 한다(같은 말 반복=잔소리, 금지).\n`
    : '';
...
${ctx}
${scenarioBlock}${chronicBlock}${snackBlock}
작성 지침:
- letter: 3~4문장(간식 평가를 녹일 땐 최대 5문장). ① 노력 인정 + 거부는 정상(반복 노출) 안심 → ② 위 데이터에서 읽은 사실 1개(우리 분석값·시계열만) → ③ 오늘의 행동 1개. 행동은 '집 아침·저녁 끼니' 또는 '기관에서 거부한 식재료를 집에서 부담 없이 다시 만나기'에서만. 어린이집·유치원 급식 메뉴 변경 요청 금지. 점수·등급 금지.
