...
// ★ EPIC C·D — Letter B 품질축 스캔·검증 합본(순수 함수만 import — 배선은 이 파일의 composeLetterB).
import { combineForVerify } from './coachQuality';
import { qualityScan } from './coachGrounding';
...
// ── ⭐ LLM 사용량 계측(유지비용 실측) ──────────────────────────────────────
// callClaude가 응답 usage를 여기 적재. 크론이 자녀 루프 시작에 resetUsage(), 종료(finally)에 getUsage()로 합산→llm_usage.
...
export type LetterInput = {
  ...
  // ── ★ EPIC C grounding 필드(Letter B 전용·전부 optional) ─────────────────────────
  // ⚠️ 이 필드들은 buildLetterUserB/composeLetterB(merged 작문 경로)만 읽는다.
  //    기존 buildLetterUser/composeLetter/verifyFacts/generateLetter/verifyLetter는 이 필드를 절대 읽지 않으므로
  //    groundingMode를 채워도 Letter A(대조군) 경로 산출은 byte 무변경이다(C-01).
  groundingMode?: GroundingMode | null;   // 'merged'면 Letter B 작문 경로(재료=결정론·문장=LLM·사실=카드)
  teachingGuide?: string | null;          // 두뇌 가이드(buildTeachingGuide 직렬화) — 고정 블록 아님(가이드)
  materials?: string | null;              // 검증 재료(selectDailyMaterials→serializeMaterials) — LLM은 이 안에서만
  materialFoods?: string[] | null;        // qualityScan 화이트리스트(재료에 등장하는 음식명·식재료명)
  mirror?: string | null;                 // 식단 거울(coachFacts.buildMealMirror) — byte 복사 금지·데이터로 주입
  onboardingMode?: boolean;               // 기록<3일 — 분석 끄고 입력안내+즉효팁만(환각 차단)
  loggedDaysTotal?: number;               // 전체 기록 일수(온보딩 안내 문구용)
  missingInputHints?: string[] | null;    // 빠뜨린 입력 안내 목록(buildOnboardingDecision 산출)
};

// ★ EPIC C — Letter B(merged) 작문 모드 신호. 재료=결정론·문장=LLM·사실=카드.
...
검증된 추천(테이블 근거 — '[오늘 타깃] 식재료→인기 음식' + 잘 먹는 식재료→사촌·궁합. 음식·사촌·궁합은 이 목록 안에서만 골라라. ⚠️ 추천 식재료는 '오늘 부족 식품군(타깃)'에 속한다 — "○○ 채소가 부족하다"면서 달걀·고기처럼 다른 식품군을 권하지 마라: 부족 프레이밍과 추천 식재료의 식품군을 반드시 일치시켜라):
${b.bridgeFacts || '파악 중'}
...
${ctx}
${factsBlock}${scenarioBlock}${planTargetBlock}${openBlock}${actionBlock}${arcBlock}${chronicBlock}${snackBlock}${moveBlock}${regenBlock}${fixBlock}${reengageBlock}${safetyBlock}
작성 지침:
...
- ⭐ 영양거울 + 음식 추천(항상 포함 — 오늘 코칭이 환경·자율성·식감이어도 빼지 마라·이사님 2026-06-15): 편지 안에 (1) **영양거울 한 구절** — '집에서 부족한 식품군'을 자연스럽게 비춰라(예 "집 아침·저녁엔 콩류·주황초록 채소가 좀 비어요"). 부족 식품군이 없으면 영양이 잘 채워지고 있음을 짧게. (2) **음식 추천 1개** — 위 '검증된 추천' 목록의 음식 이름 1개를 부드럽게 제안하라(예 "환경이 자리잡으면 두부를 순두부찌개로 곁들여보면 좋아요"·목록 밖 지어내기 금지). ⚠️ '오늘 꼭 해달라는 행동'은 1개(오늘의 핵심 레버)뿐 — 영양거울·음식 추천은 강요가 아니라 정보·앞으로의 부드러운 선택지로(두 번째 숙제로 만들지 마라). 환경 편지면 "환경이 자리잡으면 ~"처럼 미래 제안으로.
...
export async function generateLetter(input: LetterInput, model?: string): Promise<{ letter: string; oneliner: string }> {
  const out = await callClaude(buildLetterUser(input), 700, SYSTEM_COACH, model || COACH_MODEL_HAIKU);
...
2) 행동 1개: 행동 제안이 2개 이상이거나 '오늘의 계획 무브'와 무관한 별개 행동이면 위반. [음식 행동 금지] 표시가 있으면 새 식재료·식품군의 섞기/곁들이기/메뉴 제안은 전부 위반.
...
export async function verifyLetter(p: { letter: string; facts: string; noFoodAction: boolean; noRediagnose: boolean }): Promise<LetterVerify> {
  ...
  try {
    const out = await callClaude(user, 350, SYSTEM_VERIFY);
    return {
      ok: out.ok !== false,
...
export async function composeLetter(p: {
  ...
  return { letter: gen.letter, oneliner: gen.oneliner, plan, scenarioId: scenario.id, scenarioLabel: scenario.label, coachRegen, verify, modelUsed: model || COACH_MODEL_HAIKU };
}

// ══════════════════════════════════════════════════════════════════════════════
// ★ EPIC C — Letter B(merged) 작문 경로. 재료=결정론·문장=LLM·사실=카드.
//   ⚠️ 아래 함수들만 grounding 필드를 읽는다. buildLetterUser/composeLetter/verifyFacts/generateLetter/
//      verifyLetter(Letter A 대조군)는 절대 호출·수정되지 않으며 byte 무변경이다.
// ══════════════════════════════════════════════════════════════════════════════

/**
 * Letter B 화이트리스트 — qualityScan(재료 밖 음식명)·offMaterialFood 대조용 음식·식재료명.
 *  materialFoods가 있으면 그대로, 없으면 favoriteFoods를 합친다(LLM이 본문에 써도 되는 것만).
 */
export function allowedFoodsFromBridge(b: LetterInput): string[] {
  const out = new Set<string>();
  for (const f of b.materialFoods || []) if (f) out.add(String(f));
  for (const f of b.favoriteFoods || []) if (f) out.add(String(f));
  return [...out];
}

/**
 * ★ buildLetterUserB — merged(Letter B) 프롬프트 빌더. 재료·거울·가이드를 '데이터/가이드'로 주입하되
 *   문장·도입·톤은 LLM 자유(고정 슬롯·byte 거울 금지). 온보딩(기록<3일)은 분석 블록을 전부 생략한다.
 *   ⚠️ buildLetterUser(Letter A)와 완전 별개 — A 경로는 이 함수를 거치지 않는다(대조군 보존).
 */
export function buildLetterUserB(b: LetterInput): string {
  const name = (b.childName || '아이').toString().slice(0, 20);
  const age = AGE_LABEL[b.ageBand || ''] || '유아';

  // ── C-05 — 온보딩(기록<3일) 최우선 분기: 분석·결핍·시계열·재료 전부 생략(환각 차단) ─────
  if (b.onboardingMode) {
    const days = b.loggedDaysTotal ?? 0;
    const hints = (b.missingInputHints && b.missingInputHints.length)
      ? b.missingInputHints
      : ['끼니 기록', '거부한 음식', '식사 환경'];
    const tip = pickTip((b.recentLoggedDays ?? days) + name.length);
    const fixBlock = b.fixNotes?.length
      ? `\n[⚠️⚠️ 재작성 — 직전 생성이 발행 전 검증에서 걸렸다. 아래 위반을 전부 제거하고 다시 써라:\n${b.fixNotes.slice(0, 6).map((v) => `· ${v}`).join('\n')}]\n`
      : '';
    return `[온보딩 — 기록이 ${days}일뿐이라 분석하지 않는다. ⚠️ 결핍·패턴·영양 분석을 절대 하지 마라(데이터가 없어 환각이 된다). 모자란 영양·식품군·과거 빈도를 지어내지 마라.]
아이: ${name} (${age})
오늘 편지의 역할은 두 가지뿐이다:
(1) 빠뜨린 입력을 부드럽게 안내 — 아래 항목이 채워지면 더 정확히 봐드린다고: ${hints.join(' · ')}.
(2) 배경 없는(데이터에 기대지 않는) 즉효 편식 팁 1개를 자연스럽게 녹여라.${tip ? `\n   오늘의 팁: ${tip}` : ''}
${fixBlock}
[규칙] 따뜻한 존댓말 3~4문장. 점수·등급 금지. 행동 제안은 한 가지(P7)만, 작고 오늘 가능하게. 위 안내는 편지 맨 끝에 부담 없는 한 줄로. 죄책감·압박 금지.
반드시 JSON만: {"letter": "...", "oneliner": "..."}`;
  }

  // ── 분석 모드 — 재료(결정론)·거울(데이터)·가이드 주입 + raw timeseries 강등 ──────────────
  const reds = (b.reds || []).slice(0, 8);
  const refused = (b.refused || []).slice(0, 10);
  const home = (b.homeRefused || []).slice(0, 10);
  const daycare = (b.daycareRefused || []).slice(0, 10);
...
최근 거부한 음식(전체): ${refused.length ? refused.join(', ') : '없음'}
집에서 거부(부모가 재노출 가능): ${home.length ? home.join(', ') : '없음'}
기관에서 거부(집에서 재노출로 도울 수 있음): ${daycare.length ? daycare.join(', ') : '없음'}${b.attendsDaycare ? `
등원: 어린이집·유치원에 다녀 평일 점심·간식은 기관에서 먹습니다(메뉴는 부모가 못 바꿈). 행동은 집 아침·저녁 끼니와 기관 거부 식재료의 집 재노출에만.` : ''}${(b.recentWindowDays && b.recentLoggedDays != null && b.recentLoggedDays < b.recentWindowDays) ? `
기록 현황(P9): 최근 ${b.recentWindowDays}일 중 ${b.recentLoggedDays}일 기록됨(${b.recentWindowDays - b.recentLoggedDays}일 공백). 이 사실로 횟수·날짜를 더 지어내지 말 것.` : ''}
부모 메모: ${fenceNotes(b.notes || [])}`;

  // C-02 — 재료(결정론) 블록. materials 없으면 기존 bridgeFacts로 폴백(B-에픽 미완 안전).
  const materialsBlock = b.materials
    ? `\n[오늘의 재료 — 코드가 매일 검증·회전해 확정한 것. ⚠️ 음식·식재료·조합은 이 목록 안에서만 골라라 — 목록 밖 조합/음식명을 지어내면 괴식·환각이다. ★ 무엇을 추천할지는 이미 정해졌다 — 너는 어떻게 따뜻하게 쓸지만 결정하라]\n${b.materials}\n`
    : (b.bridgeFacts
      ? `\n[검증된 추천(테이블 근거 — 음식·사촌·궁합은 이 목록 안에서만 골라라. 목록 밖 조합을 지어내지 마라)]\n${b.bridgeFacts}\n`
      : '');

  // C-03 — 거울(데이터) 블록. byte 복사 금지·사실(메뉴·결핍군)은 보존.
  const mirrorBlock = b.mirror
    ? `\n[식단 거울 — 아이가 실제 먹은 것/영양 신호. ⚠️ 그대로 베끼지 말고 너의 표현으로 1~2문장에 녹여라(byte 복사 금지). 단, 사실(메뉴·결핍군)은 바꾸지 말 것]\n${b.mirror}\n`
    : '';

  // C-04 — 두뇌 가이드(teachingGuide) 블록. 있으면 arcBlock(weeklyArc) 생략.
  let guideBlock = '';
  if (b.teachingGuide) {
    guideBlock = `\n[오늘의 코칭 방향 — 코드/Sonnet가 정한 교육 의도. ⚠️ 의도·초점은 따르되 문장·도입·예시·톤은 자유롭게(고정 문구 복사 금지). 행동(ⓒ)은 한 가지(P7)만 — 둘째 행동을 얹지 마라]\n${b.teachingGuide}\n`;
  } else if (b.weeklyArc) {
    const a = b.weeklyArc;
    const goal = `'${a.behaviorGoal}'${a.implIntention ? ` (${a.implIntention} 같은 구체적 순간에)` : ''}`;
    guideBlock = `\n[이번 주 코칭 방향 — 톤·초점. 행동(ⓒ)은 한 가지만] 이번 주 부모가 시도하면 좋은 작은 변화: ${goal}. 오늘은 이 초점으로 따뜻하게 쓰되, 도입·표현은 자유롭게.\n❌ '미션·과제·목표·챌린지' 단어 금지.\n`;
  }

  // C-06 — raw timeseries 강등(사실 인용 금지·톤 참고용) + factCards 유일 사실 출처 강조.
  const tsBlock = `시계열(참고용 — ⚠️ 여기서 횟수·날짜·패턴을 인용하지 마라. 사실 주장은 아래 사실 카드에서만): ${ts.length ? ts.join(' / ') : '없음'}`;
  const factsBlock = b.factCards?.length
    ? `\n[⚠️ 사실 카드 — 코드가 시계열로 계산한 '말해도 되는 사실'의 전부. 편지의 모든 사실(빈도·횟수·환경·거부·메뉴)은 이 카드에만 근거하라 — timeseries 원문 숫자 인용 금지, 적힌 시계열 라벨대로(단발 1회·하루 관찰을 패턴으로 일반화 금지)]\n${b.factCards.map((c) => `· ${c}`).join('\n')}\n`
    : '';

  const history = past.length
    ? `\n[최근 보낸 코칭 편지 — 최근 것부터. ⚠️ 이것들과 도입·표현·식재료·무브가 겹치지 마라. 날짜·경과일 계산 금지]\n${past.map((p, i) => `[${i === 0 ? '바로 직전' : i + 1 + '번째 전'} 편지]\n${p.letter}`).join('\n\n')}\n결론: 매일 새 관점·새 표현. 동어반복은 잔소리이며 금지.\n`
    : '';

  const regenBlock = b.regenAvoid
    ? `\n[⚠️⚠️ 재작성 — 직전 생성이 아래 편지와 너무 비슷했다(특히 도입). 첫 문장을 같은 일화·소재·표현으로 시작하지 말고 완전히 다르게 열어라. 짚는 식재료·통로·무브도 겹치지 않게.\n아래 편지:]\n${b.regenAvoid.slice(0, 600)}\n`
    : '';
  const fixBlock = b.fixNotes?.length
    ? `\n[⚠️⚠️ 재작성 — 직전 생성이 발행 전 검증에서 걸렸다. 아래 위반을 전부 제거하고 다시 써라:\n${b.fixNotes.slice(0, 6).map((v) => `· ${v}`).join('\n')}]\n`
    : '';

  const safetyBlock = `\n[⚠️ 안전·정직 가드]\n· 입력에 없는 증상(사레·헛구역질·뱉음·구토)을 사실로 단정하지 마라. 데이터에 없는 시점·기간을 쓰지 마라.\n· 단 하루의 기록·메모를 '계속 반복되는 리듬/패턴'으로 일반화하지 마라.\n· 김치류(김치·깍두기)에 다른 재료를 섞으라고 하지 마라(괴식). 아이에게 '-시-' 존대 금지.\n· 단 과일·우유에 콩·두부·생선·멸치를 섞지 마라(괴식). 국·탕·찌개는 '마시다' 아니라 '먹다'.\n· 밀(가루)·쌀(멥쌀·현미)은 날것으로 섞지 말고 '먹는 형태'로만(밀→빵·면·떡, 쌀→밥·떡).\n· 점수·등급·체중·살·다이어트·BMI 단어 금지(유아 체중은 민감).\n`;

  return `${history}[이번 주 상황 — 아래 사실·재료만 사용. 영양/과거/숫자를 추가로 지어내지 말 것]
${ctx}
${tsBlock}
${factsBlock}${materialsBlock}${mirrorBlock}${guideBlock}${regenBlock}${fixBlock}${safetyBlock}
작성 지침(merged — 재료=결정론·문장=LLM):
- letter: 3~4문장. 담을 요소: ⓐ 따뜻한 인정/공감 ·${b.mirror ? " ⓑ 위 '식단 거울'을 너의 표현으로 풀어 쓴 1~2문장(byte 복사 금지·사실 보존) ·" : ''} ⓒ '오늘의 재료'·'코칭 방향'에 맞춘 오늘의 행동 1개(P7). 도입·순서·톤은 매일 자유롭게 — 고정하지 마라.
- ⚠️ 음식·사촌·궁합·조합은 위 '오늘의 재료'(없으면 '검증된 추천') 목록에 적힌 것만 써라. 목록 밖 음식명·조합을 지어내지 마라(괴식·환각). 레시피·조리법 상세(양념·불세기)는 쓰지 마라.
- ⚠️ 곁들임/섞기 가드: 타깃 식재료를 어떤 음식에 '섞어/곁들여/같이'라고 쓰려면 위 '검증된 조합'에 그 음식이 있어야 한다. 잘 먹는 음식이라도 검증 목록에 없으면 거기에 섞으라고 쓰지 마라(예: '미역국에 당근 섞기'·'떡에 달걀'은 실제 식단 근거 없음). '곁들임 규칙' 줄이 '섞지 마라'면 타깃을 작은 양으로 그 자체를(또는 또래 인기 음식 이름으로) 권하라.
- ⚠️ 모든 사실 주장(빈도·횟수·환경·거부·메뉴)은 '사실 카드'에 적힌 것만, 적힌 시계열 라벨대로. timeseries 원문 숫자를 직접 인용하지 마라.
- 등원 아동: 칭찬·평가는 '집 끼니' 기준. 전체가 기관 급식 덕이면 솔직히 밝히고 집 부족에 한 걸음을 둬라. 어린이집·유치원 급식 메뉴 변경 요청 금지.
- oneliner: 최근 식단 진단 한 줄(잘하는 점 + 신경 쓸 점 1개 + 방법론 근거 한 조각, 격려 톤).
반드시 JSON만: {"letter": "...", "oneliner": "..."}`;
}

/** ★ verifyFactsB — merged 검증 '제공 데이터'에 재료·거울을 합본(무검증 채널 봉합 · C-11). verifyFacts(A)는 무변경. */
export function verifyFactsB(b: LetterInput): string {
  return [
    b.factCards?.length ? `사실 카드(시계열 라벨 — 빈도 주장의 기준): ${b.factCards.join(' / ')}` : '',
    `시계열(참고용): ${(b.timeseries || []).join(' / ') || '없음'}`,
    `부족 영양소: ${(b.reds || []).join(', ') || '없음'} · 부족 식품군: ${(b.missing || []).join(', ') || '없음'}`,
    `잘 먹는 음식: ${(b.favoriteFoods || []).slice(0, 8).join(', ') || '파악 중'} · 거부: ${(b.refused || []).join(', ') || '없음'}`,
    b.materials ? `검증된 재료(LLM이 쓸 수 있는 것 전부): ${b.materials}` : '',
    b.mirror ? `식단 거울 사실: ${b.mirror}` : '',
    b.teachingGuide ? `오늘의 코칭 방향: ${b.teachingGuide}` : '',
    `등원: ${b.attendsDaycare ? '예(평일 점심·간식은 기관에서 먹음)' : '아니오'}`,
  ].filter(Boolean).join('\n');
}

/**
 * ★ composeLetterB — Letter B(merged) 작문 경로. 재료·거울·가이드·온보딩을 적용해 생성하고,
 *   det·품질·유사도·검증 가드를 전부 통과시킨다(가드 대칭 S1). LLM 생성기는 주입 가능(테스트 결정론).
 *   route(EPIC E)가 호출한다. composeLetter(A)와 시그니처·본문 별개 — A는 무변경.
 *
 *   ⓐ generateLetter(buildLetterUserB) → ⓑ [letterDeterministicBad ‖ qualityScan(combineForVerify)] 위반 시
 *      재생성(최대 2회·timeLeft) → ⓒ verifyLetter(combineForVerify 합본=무검증 채널 봉합) → ⓓ 품질 재생성(최대 2회).
 *   온보딩은 유사도·품질 재생성을 최대 1회로 제한(데이터 빈약).
 */
export async function composeLetterB(p: {
  base: LetterInput;                 // groundingMode='merged' 가정(materials/mirror/teachingGuide/onboarding 포함)
  detInput?: string;                 // 결정론 안전가드 입력 텍스트(시계열+메모+거부)
  detForbid?: RegExp | null;
  deadlineMs?: number;               // S7 — 넘기면 추가 콜 생략(발행 우선)
  daySeed?: number; cidHash?: number;
  scenarioId?: string;               // det 가드 시나리오 컨텍스트(없으면 'nutrient-gap'류 일반)
  model?: string;
  /** ⭐ 테스트 주입 — LLM 생성기(기본 generateLetter). 가드 루프는 동일하게 돈다. */
  gen?: (input: LetterInput, model?: string) => Promise<{ letter: string; oneliner: string }>;
  /** ⭐ 테스트 주입 — 의미 검증자(기본 verifyLetter). */
  verifyFn?: (q: { letter: string; facts: string; noFoodAction: boolean; noRediagnose: boolean }) => Promise<LetterVerify>;
}): Promise<{ letter: string; oneliner: string; plan: null; scenarioId: string; scenarioLabel: string; coachRegen: boolean; verify: { ok: boolean; violations: string[]; regen: boolean } | null; quality: { violations: string[]; regen: boolean } | null; modelUsed: string }> {
  const base = p.base;
  const onboarding = !!base.onboardingMode;
  const sid = p.scenarioId || 'nutrient-gap';
  const detInput = p.detInput || [(base.timeseries || []).join(' '), (base.notes || []).join(' '), (base.refused || []).join(' ')].join(' ');
  const genInjected = p.gen;   // 주입(테스트)이면 그것을, 아니면 callClaude(buildLetterUserB) 실경로.
  const verifyL = p.verifyFn || ((q: { letter: string; facts: string; noFoodAction: boolean; noRediagnose: boolean }) => verifyLetter(q));
  const model = p.model || (base.weeklyArc?.stage === 'intro' ? (process.env.COACH_INTRO_MODEL || 'claude-sonnet-4-6') : undefined);

  // merged 프롬프트는 buildLetterUserB가 만든다 — genFn에 그 입력을 주려면 base를 그대로 넘긴다(generateLetter가
  //   buildLetterUser를 부르므로, B 경로는 genFn=generateLetterB 합성). composeLetterB는 buildLetterUserB로 직접 생성.
  const letterInput: LetterInput = { ...base, groundingMode: 'merged' };
  const allowedFoods = allowedFoodsFromBridge(base);
  const materialFoods = (base.materialFoods && base.materialFoods.length) ? base.materialFoods : allowedFoods;

  const timeLeft = () => !p.deadlineMs || Date.now() < p.deadlineMs;
  const detBad = (L: string) => letterDeterministicBad(L, sid, detInput) || !!(p.detForbid && p.detForbid.test(L));
  // 품질축은 본문+거울+추천 합본으로 검사(무검증 채널 봉합) — combineForVerify.
  ...
  // ⓒ 유사도 가드(보조) — pastLetters 있을 때만. 온보딩은 최대 1회.
  let simBadness: (t: string) => number = () => 0;
  const pastLetters = base.pastLetters || [];
  if (pastLetters.length) {
    const fc = (s: string) => (s || '').replace(/\s+/g, '').slice(0, 25);
    const wholeMax = (t: string) => Math.max(...pastLetters.map((q) => letterSimilarity(t, q.letter)));
    const openMax = (t: string) => Math.max(...pastLetters.map((q) => letterSimilarity(fc(t), fc(q.letter))));
    simBadness = (t: string) => Math.max(wholeMax(t) / 0.45, openMax(t) / 0.40);
    const closest = (t: string) => pastLetters.reduce((a, q) => letterSimilarity(fc(t), fc(q.letter)) > letterSimilarity(fc(t), fc(a.letter)) ? q : a, pastLetters[0]);
    let bestBad = simBadness(gen.letter);
    const simCap = onboarding ? 1 : 2;
    for (let attempt = 0; attempt < simCap && bestBad >= 1 && timeLeft(); attempt++) {
      const g = await generateLetterB({ ...letterInput, regenAvoid: closest(gen.letter).letter }, model, genInjected);
      const bad = simBadness(g.letter);
      if (bad < bestBad && !detBad(g.letter) && (onboarding || qScan(g.letter).length === 0)) {
        gen = g; bestBad = bad; coachRegen = true;
      }
    }
  }

  // ⓓ 의미 검증자(발행 전 1콜) — combineForVerify 합본 입력(무검증 채널 봉합). 장애 시 발행 차단 안 함.
  let verify: { ok: boolean; violations: string[]; regen: boolean } | null = null;
  try {
    if (timeLeft()) {
      const facts = verifyFactsB(letterInput);
      const noFoodAction = !!base.teachingGuide && /environment|autonomy/.test(base.teachingGuide) && !base.materials;
      const noRediagnose = !!base.weeklyArc && base.weeklyArc.stage !== 'intro';
      const v = await verifyL({ letter: verifyText(gen.letter), facts, noFoodAction, noRediagnose });
      verify = { ok: v.ok, violations: v.violations, regen: false };
      if (!v.ok && (v.violations.length || v.hint)) {
        const g = await generateLetterB({ ...letterInput, fixNotes: [...v.violations, ...(v.hint ? [v.hint] : [])] }, model, genInjected);
        const v2 = await verifyL({ letter: verifyText(g.letter), facts, noFoodAction, noRediagnose });
        if ((v2.ok || v2.violations.length < v.violations.length) && !detBad(g.letter) && simBadness(g.letter) < 1) {
          gen = g; coachRegen = true; verify = { ok: v2.ok, violations: v2.violations, regen: true };
        }
      }
  ...
  return {
    letter: gen.letter, oneliner: gen.oneliner, plan: null,
    scenarioId: sid, scenarioLabel: 'merged', coachRegen, verify, quality,
    modelUsed: model || COACH_MODEL_HAIKU,
  };
}

/**
 * generateLetterB — buildLetterUserB로 merged 프롬프트를 만들어 LLM 생성(+퇴고). composeLetterB 전용.
 *  injected가 있으면(테스트) 그 생성기에 입력을 그대로 넘긴다(buildLetterUserB는 우회 — 입력 단언용).
 *  injected가 없으면(런타임) callClaude(buildLetterUserB)로 직접 — Letter A의 buildLetterUser는 거치지 않는다.
 */
async function generateLetterB(
  input: LetterInput,
  model: string | undefined,
  injected?: (input: LetterInput, model?: string) => Promise<{ letter: string; oneliner: string }>,
): Promise<{ letter: string; oneliner: string }> {
  if (injected) return injected({ ...input, groundingMode: 'merged' }, model);
  const out = await callClaude(buildLetterUserB(input), 700, SYSTEM_COACH, model || COACH_MODEL_HAIKU);
  const raw = (out.letter as string) || '';
  const letter = raw ? await proofreadLetter(polishKo(raw)) : raw;
  return { letter, oneliner: (out.oneliner as string) || '' };
}
