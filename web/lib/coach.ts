...
${ctx}
${scenarioBlock}${planTargetBlock}${openBlock}${actionBlock}${arcBlock}${chronicBlock}${snackBlock}${moveBlock}${regenBlock}${fixBlock}${reengageBlock}${safetyBlock}
작성 지침:
...
[검사 항목]
1) 사실 근거: 편지의 사실성 주장(패턴·리듬·횟수·시점·전환·먹었다/안 먹었다·기관에서의 일)이 제공 데이터에 직접 근거하는가. 부모 메모는 주관 관찰일 뿐 — 하루치 메모를 '반복되는 리듬·패턴'으로 일반화하면 위반. 데이터와 모순되는 단정(예: 점심 기록이 있는데 '점심을 거른다')은 위반.
...
[위반 아님] 따뜻한 일반론(거부는 정상·8~10회 노출 같은 방법론 일반 지식)·표현 차이·제공 데이터의 자연스러운 요약. 확실한 위반만 잡고, 애매하면 ok.
