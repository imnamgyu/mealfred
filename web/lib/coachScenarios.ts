...
export const SCENARIOS: CoachScenario[] = [
  ...
  },
  {
    ...
    promptHint: `거부했던 식재료(특히 기관에서 거부한 것)가 있다. 한 번 거부로 빼버리면 친해질 기회가 사라짐을 짚고, 격일로 아주 작은 양을 집에서 다시 만나게 하는 타이밍 1개. ⭐ 위 시계열에 'OO를 N번 만났고 마지막이 M일 전 — 지금 다시 권하기 좋은 시점' 사실이 있으면 그 숫자(횟수·일수)를 그대로 인용해 '왜 지금이 적기인지'를 데이터로 설명하라(없는 숫자는 지어내지 말 것). 무서워하면 보기→만지기→냄새 사다리부터.`,
