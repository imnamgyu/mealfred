...
export function compileFactCards(p: { rows: FactRow[]; today: string }): { cards: FactCard[]; noteCards: string[]; forbidParts: string[] } {
  const { rows, today } = p;
...
export function compileFacts(p: { rows: FactRow[]; today: string }): { cards: string[]; noteCards: string[]; forbidParts: string[] } {
  ...
  return { cards: r.cards.map((c) => c.text), noteCards: r.noteCards, forbidParts: r.forbidParts };
}
