  ...
  properties: {
    ...
    items: {
      ...
      items: {
        ...
        required: ['date', 'day', 'menu', 'ingredients', 'note'],
...
규칙:
1. 실제 메뉴만 items로 만드세요. OCR 글자가 깨졌으면 한국 급식에서 실제 쓰는 메뉴로 교정하고, 도저히 모르겠으면 버리세요(환각 금지).
...
7. note: 그 메뉴가 아이에게 주는 핵심 포인트를 **메뉴마다 다르게** 짧게(공백 포함 6~16자) 적으세요. 좋은 점은 영양·효익으로(예: "철분·엽산, 빈혈예방", "칼슘·단백질, 키·뼈", "비타민C, 면역", "오메가3, 두뇌"), 초가공식품·과자·당류·튀김 위주면 주의로(예: "초가공·당류 주의", "튀김, 기름 주의"). 흰밥·물·차처럼 영양 포인트가 약하면 "탄수, 에너지"처럼 간단히.

items 원소: {date(날짜 숫자 1~31, 모르면 ""), day(요일 월화수목금토일, 모르면 ""), menu(교정한 메뉴명), ingredients(분해 배열), note(위 7번 한 줄 평가)}.
