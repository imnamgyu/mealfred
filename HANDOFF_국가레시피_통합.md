...
> **📌 변경 이력(2026-06-14, commit 2682137 반영 — 본문 수치·표현 모두 최신)** — 교차채널 과일 필터를 구현·배포: `gen-food-graph.py` 식판 병합에서 과일(cat='과일') 쌍 116개 배제 → 과일×주메뉴 strong 11→0(`소고기+포도·닭고기+바나나` 등 제거, 잔존 `딸기+빵`=레시피 출처 정상). 그 결과 **pair 468→451 · 식판 신규 106→89 · 승급 37→35 · 합 643→626**, 회귀 테스트 갱신·835 그린. **남은 작업: 유제품(우유·치즈)×끼니 교차채널 컷은 아직 미적용 — 새 식단표/끼니묶음 원천 합류 전 내가 추가한다.**
...
> 1. **교차채널 노이즈 필터 — 과일은 구현·배포 완료**(`2682137`: `gen-food-graph.py` 식판 병합에서 과일 cat='과일' 쌍 116개 배제 → `소고기+포도·두부+포도·닭고기+바나나` 등 과일×주메뉴 strong 11→0). **단 유제품(우유·치즈)×끼니 교차채널은 아직 미적용.** 신규 식단표/끼니묶음 원천은 식판축 합류 전 이 필터(과일+유제품)를 반드시 통과시킬 것.
...
- **식판축(NEIS 트레이 공통출현) — upgrade-only 병합**: `pull-neis-tray.py` → `/tmp/neis-trays.json` → `gen-neis-tray-cooccur.py` → `/tmp/neis-tray-pairs.json` → gen-food-graph가 병합.
  ...
  - 병합 순서(중요): gen-food-graph는 GRANK 비교 **이전에** 매칭되는 기존 edge에 `e['tray']=tg`를 **무조건 set**한다. 그 다음 `GRANK[tg]>GRANK[e.grade]`일 때만 `e.grade=tg`(**강등 금지**), tg=='strong'&strength<2면 strength=2, `n_tray_up++`(실측 35).
  - 기존 edge 없고 tg=='strong'이면 새 pair edge 추가(`strength:3,grade:strong,src:'tray',tray:'strong',basis:'같은 끼니 식단 {c}회'`), `n_tray_new++`(실측 89). **medium/weak tray-only 쌍은 버려짐. 과일(cat='과일') 쌍은 교차채널 필터로 배제(116).**
  - ⭐**`tray` 필드 보유 edge 수(실측 190) ≠ 승급 edge 수(35) ≠ tray-only 신규(89)**. `e['tray']=tg`는 GRANK 비교 전 무조건 set이라, "tray 필드 = 승급"이 아니다. 재생성 후 카운트 해석 시 혼동 금지.
...
6. **교차채널 노이즈**: 간식채널(과일·유제품)×끼니 단백질/곡물의 같은-끼니 동시출현은 곁들임 궁합 아님. ✅**과일 필터는 구현·배포됨**(2682137, 과일 cat 쌍 식판 병합 배제). ⚠️**유제품(우유·치즈)×끼니는 아직 미적용**.
...
5. **교차채널 노이즈 필터 — 과일은 구현됨, 유제품은 추가 예정**: 과일(cat='과일') same-tray 쌍 컷은 배포됨(2682137). 유제품(우유·치즈)×끼니 컷은 내가 추가한다. 신규 원천이 '식단표/끼니 묶음'이면 이 필터(과일+유제품)를 거쳐야 합류 — 레시피(한 그릇) 동시출현은 OK지만 식판/끼니류는 유제품 필터까지 완료 후.
...
5. **카운트 증가분의 출처 분해 후 테스트 드리프트 갱신(의무)**:
   ...
   - I-05-4: pair `toBe(451)` · bridge `toBe(175)` · `edges.length toBe(626)`. **주석에 `451=362(레시피)+89(식판 strong 신규·과일 교차채널 제외 후)` split이 하드코딩돼 있음 — 새 합계로 무지성 교체 금지(오염 tray-strong을 그린으로 승인할 위험).**
...
- ❌ **교차채널 노이즈 통과 금지**: 학교 식판은 후식 과일이 매끼 단백질과 함께 올라 lift>1.2라 **lift 게이트로 안 걸러진다**(`소고기+포도·닭고기+바나나` 등). ✅**과일 컷은 구현·배포됨**(2682137 — `gen-food-graph.py` 식판 병합에서 과일 cat='과일' 쌍 116 배제, 과일×주메뉴 strong 11→0). ⚠️**유제품(우유·치즈)×끼니 컷은 미적용 — 내가 추가 예정.** **신규 식단표/끼니묶음 원천은 과일+유제품 필터가 둘 다 완료된 뒤** 식판축에 합류(레시피 한 그릇 원천은 무관·우선 진행 OK).
