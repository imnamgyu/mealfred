...
### 🚨 배포 규칙 — 자잘한 변경은 모아서 커밋·푸시
- **자잘한 수정(카피·디자인·버그 한 줄 등)은 한 건씩 push하지 말고 모아서 한 번에 커밋·푸시한다.**
- **이유**: 한 리포(`imnamgyu/mealfred`)에 Vercel 프로젝트가 **2개**(`mealfred` 정적 + `mealfred-app` 앱) 연결돼 있어 **`git push` 1번 = 배포 2개** 소비. 무료(Hobby) 플랜은 **하루 배포 100개 한도**(`api-deployments-free-per-day`) → 분당 커밋하면 순삭됨.
- **2026-05-28 실제 사고**: 동시 세션이 2~3분 간격으로 커밋을 쏟아내 100/일 한도 초과 → mealfred.com 배포가 24h 막힘(히어로 수정이 라이브에 안 올라감). 한도는 계정 단위라 `vercel --prod` CLI 수동배포도 같이 막힘. 24h 롤링 리셋 후 새 push 1번 필요.
- **추가 절감책**: 각 Vercel 프로젝트 Settings→Git→**Ignored Build Step**으로 관련 경로 변경 시만 빌드(정적=루트 HTML, 앱=`web/`) → 배포 소비 ~절반.
- **동시 세션 주의**: 여러 세션이 같은 `.git`에서 동시에 커밋하면 한도 소비가 합산된다. 푸시 빈도 조율 필요.
...
### 2026-05-28 (밤) — 끼니별 멘트·가공2단계·가정보충 개편·바이럴 URL·문서 8축 정합·로딩 UX

- **끼니별 평가 멘트(note)**: 룰 기반 식재료 매칭이 똑같은 블롭 양산 → Haiku가 메뉴별 note 직접 작성(영양·효익 / 초가공·당류 주의). 표는 note 우선, 없으면 룰 폴백. (점수=룰, 멘트=LLM)
- **가공식품 2단계**: 초가공(소시지·과자·음료·라면, 강한 감점) vs 일반가공(어묵·만두·피자, 0.3 가중). 카레·돈가스·튀김 등 집밥은 제외(너무 빡빡 완화). 표 색: 초가공=빨강·가공=주황
- **가정보충 개편**: "필수" 단어 제거 → "🎒 초등학교 입학 전까지" 프레이밍 + 카테고리별 식단 미등장 식재료 빈도순 최대 5개. 멥쌀→현미, 밀→통밀빵. 고추 제거. **연령 안전 필터**: 만3세미만/3-4세는 땅콩 제외·견과 "(갈아서)"
- **레시피 CTA**: 가정보충 직후 프로미넌트 버튼("위 식재료로 우리 아이 레시피 받아보기") → care.html
- **메뉴별 식단분석 표**: 명칭 "OCR 추출 식단 상세"→"메뉴별 식단분석", 모달 맨 아래(바이럴 아래) 이동
- **바이럴 결과 URL**: `app.mealfred.com/eval/[id]` (Next 16 route) — generateMetadata 동적 OG(제목에 "B+ 88점 · 우리 아이 식단표 평가"), 봇은 OG만·사람은 ?r= 결과로 redirect, 3일 만료. shareResult가 이 URL 사용. 봇 테스트 통과. (등급카드 동적 OG 이미지는 satori 한글폰트 검증 후 추가 예정)
- **PNG 카드 저장 제거** → 결과 링크 공유로 일원화 (html2canvas 느림)
- **문서 8축 단일 정합**: docs/roadmap(M6)/architecture/architecture-detailed(ALG-EVAL-01·05)/dev-spec/test-scenarios/engines-deep/care — 7축·9축 혼재 → 8축. OCR 엔진: 식단표=CLOVA+Claude, 음식사진=Vision 구분. Satter 정성신호(자율성 등)는 8축 점수 아니라 AI 코치로 분리
- **분석 로딩 UX**: XHR 업로드 진행률(실제%) + 서버처리(30~50초) 40→95% 점근 프로그래스 + 경과초·예상시간. 멈춘 느낌 제거
- 비용 실측: CLOVA(건당 ~1.5원, 월 5천건 무료) + Haiku(입력4234·출력5754 = ₩46) ≈ **건당 ₩46~48**. 호출 ~44초(maxDuration 60s)
...
**끼니별 평가 멘트 (note, 배포 c0e23ba)**: 룰 기반 식재료 매칭이 "면역·피부·두뇌발달" 똑같은 블롭 양산 → 메뉴별 `note`를 Haiku가 직접 작성. 실측: 사과→"비타민C, 면역", 쇠고기무밥→"철분, 빈혈예방", 카스텔라→"초가공·당류 주의", 누룽지죽→"탄수, 에너지". 끼니마다 다르고 정확. 표는 note 우선, 없으면 룰 폴백. (rule-vs-llm: 점수=룰, 멘트=LLM)
**비용 실측 (한 달 92건 식단표)**: Haiku 입력 4,234 + 출력 5,754 토큰 = $0.033 ≈ ₩46. CLOVA는 **건당**(단어당 아님) 월 5,000건 무료·초과 1.5원/건. **합계 ≈ ₩46~48/건**. 단, 호출 **44초** 소요(maxDuration 60s 근접) — 추후 최적화 여지.
